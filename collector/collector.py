...
class TraceDataCollector(object):
    def __init__(self, host_machine_config=None, role=None):
        if host_machine_config != None:
            ...
            self.host_machine_config = utils.parse_config(
                "config/host_machine_config.json")
        self.hostname = socket.gethostname()
        self.config = self.host_machine_config[self.hostname]
        logging.basicConfig(
            level=logging.DEBUG, format='%(asctime)s | %(levelname)-1s [%(filename)s:%(lineno)d] %(message)s', datefmt='%Y-%m-%d:%H:%M:%S')
    ...
    def pcap_to_txt(self, file_full_path, output_file_name):
        if os.path.exists(file_full_path):
            os.system("tshark -r " + file_full_path +
                      " -T fields -e frame.time_epoch -e frame.len > " + output_file_name)
            os.system("rm " + file_full_path)
        else:
            self.logger.error("Output trace data: {} to {}".format(
                file_full_path, output_file_name))
...
class TraceDataCollectionClient(TraceDataCollector):
    def __init__(self, host_machine_config=None, role="client"):
        super(TraceDataCollectionClient, self).__init__(
            host_machine_config, role)
    ...
    def iperf_tcpdump_download(self, test_config):
        ...
        pcap_result_path = os.path.join(test_config["pcap_path"], "{}_{}_{}_{}".format(
            network, direction, variant, experiment_id))
        ...
        experiment_start_time = datetime.fromtimestamp(
            time.time()).strftime("%Y_%m_%d_%H_%M_%S")
        output_pcap = os.path.join(pcap_result_path, "{}_{}_{}_{}.pcap".format(
            network, direction, variant, experiment_start_time))
        output_iperf = os.path.join(pcap_result_path, "iperf_result.json")

        if variant != "udp":
            os.system(
                "tcpdump -i any tcp -s 96 src port {} -w {} > /dev/null 2>&1 &".format(iperf_port, output_pcap))
        else:
            os.system(
                "tcpdump -i any udp -s 96 port {} -w {} > /dev/null 2>&1 &".format(iperf_port, output_pcap))
        ...
        while not client_timer.is_expire():
            try:
                ...
                if variant != "udp":
                    P_iperf_client = subprocess.Popen("exec iperf3 -c {} -p {} -R -t {} -i {} --json --logfile {}".format(
                        self.server_ip, iperf_port, task_time, iperf_logging_interval, output_iperf), shell=True)
                else:
                    P_iperf_client = subprocess.Popen("exec iperf3 -c {} -p {} -R --length 1472 -u -b {}m -t {} -i {}  --json --logfile {}".format(
                        self.server_ip, iperf_port, self.udp_sending_rate, task_time, iperf_logging_interval, output_iperf), shell=True)
                P_iperf_client.wait(task_time)
                if time.time() - retry_start_time <= 5:  # assume no experiment is running, retry
                    retry_start_time = time.time()
                    self.logger.warning(
                        "Error to connect with iperf3 server, retry...")
                    time.sleep(1)
                else:
                    break
    ...
    def iperf_tcpdump_upload(self, test_config):
        ...
        if variant != "udp":
            os.system(
                "sudo sysctl net.ipv4.tcp_congestion_control={}".format(variant))
        client_timer = utils.DokiTimer(expired_time=min(task_time, 300))
        while not client_timer.is_expire():
            try:
                if "retry_start_time" not in locals():
                    retry_start_time = time.time()
                if variant != "udp":
                    P_iperf_client = subprocess.Popen("exec iperf3 -c {} -p {} -t {} -i {}  --json --logfile {}".format(
                        self.server_ip, iperf_port, task_time, iperf_logging_interval, output_iperf), shell=True)
                else:
                    P_iperf_client = subprocess.Popen("exec iperf3 -c {} -p {} --length 1472 -u -b {}m -t {} -i {}  --json --logfile {}".format(
                        self.server_ip, iperf_port, self.udp_sending_rate, task_time, iperf_logging_interval, output_iperf), shell=True)
                P_iperf_client.wait(task_time)
...
class TraceDataCollectionServer(TraceDataCollector):
    def __init__(self, host_machine_config=None, role="server"):
        super(TraceDataCollectionServer, self).__init__(
            host_machine_config, role)
    ...
    def iperf_tcpdump_download(self, test_config):
        ...
        task_time = test_config["task_time"]
        iperf_port = test_config["iperf_port"]
        os.system("sudo sysctl net.ipv4.tcp_congestion_control={}".format(
            test_config["variant"]))
        try:
            P_iperf_server = subprocess.Popen(
                "exec iperf3 -s -p {} -i {}".format(iperf_port, iperf_logging_interval), shell=True)
    ...
    def iperf_tcpdump_upload(self, test_config):
        ...
        if variant != "udp":
            os.system(
                "tcpdump -i any tcp -s 96 dst port {} -w {} > /dev/null 2>&1 &".format(iperf_port, output_pcap))
        else:
            os.system(
                "tcpdump -i any udp -s 96 port {} -w {} > /dev/null 2>&1 &".format(iperf_port, output_pcap))
