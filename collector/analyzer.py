...
class TraceDataAnalyzer(object):
    def __init__(self, web_server_config=None):
        ...
        if web_server_config != None:
            ...
            self.web_server_config = utils.parse_config(
                "config/web_server_config.json")
    ...
    def draw_graph(self, test_result_path, time_scale=None):
        ...
        df_result = pd.read_csv(test_result["trace_file_name"], names=[
        ...
        if time_scale in pre_config_timescale:
            ...
            if task_time / 3600 >= 2:  # more than 4 hours trace
                ...
                time_scale_unit = pre_config_timescale[time_scale]
        df_result = pd.DataFrame(data={"time": [int((x-start_timestamp)/time_scale_unit) for x in df_result["time"].values],
                                 "packet_size": df_result["packet_size"].values}).groupby("time").sum().reset_index()
        df_throughput = pd.DataFrame(data={"time": df_result["time"].values, "throughput": [round(
        ...
        if mean_throughput_in_mbps != 0.0:
            axs.axhline(y=mean_throughput_in_mbps,
                        label=f'Mean throughput {mean_throughput_in_mbps:.2f}', color='teal')
            axs.annotate(text=f'{mean_throughput_in_mbps:.2f}', xy=(
                max(x_plot)+1, mean_throughput_in_mbps), xytext=(8, 0), textcoords='offset points')
    ...
    def get_db_info(self):
        server_url = 'http://{}/{}fields_value.php'.format(
            self.web_server_ip, self.web_interface_dir)
    ...
    def post_file_to_server(self, test_result_path):
        ...
        server_url = 'http://{}/{}handler.php?action=Upload_Record'.format(
            self.web_server_ip, self.web_interface_dir)
        test_result = utils.parse_config(os.path.join(
            test_result_path, "experiment_result.json"))
        ...
        parameters = {"Network_ID": network_dict[network], "Variant_ID": variant_dict[variant],
                      "Direction_ID": direction_dict[direction], "Start_Time": Start_Time, "Task_Time": Task_Time, "Task_Name": Task_Name}
